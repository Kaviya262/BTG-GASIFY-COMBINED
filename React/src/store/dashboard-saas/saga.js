...
function* getSellingData({ payload: month }) {
    try {
        ...
        yield put(DashboardSaasapiFail(GET_TOP_SELLING_PRODUCT, error.message || error.toString() || 'An error occurred'));
    }
}
...
function* getEarningChartsData({ payload: month }) {
    try {
        ...
        yield put(DashboardSaasapiFail(GET_EARNING_DATA, error.message || error.toString() || 'An error occurred'));
    }
}
