...
import { ColumnGroup } from 'primereact/columngroup'; // Import for Grand Total
...
const SalesItemWise = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Pre-process to ensure clean ItemName for grouping
            const cleanedData = rawData.map((item, index) => {
                let name = item.ItemName ? String(item.ItemName) : "Unknown";
                name = name.replace(/\s+/g, ' ').trim();
                return {
                    ...item,
                    _rowId: `row_${index}`,
                    ItemName: name // Ensure grouping key is clean
                };
            });
        ...
        Object.keys(groupedData).forEach(itemName => {
            ...
            items.forEach(item => {
                excelRows.push([
                    ...
                    item.InvoiceNo,
                    item.DONumber || "",
                    item.CustomerName, // Shows Customer in sub-grid
                    parseFloat(item.Qty),
            ...
            excelRows.push(["Subtotal", "", "", "", "", totalQty, "", totalOrig, totalIDR]);
            excelRows.push([]); // Spacer
        });

        // 2. Grand Total
        const grandTotalQty = data.reduce((sum, i) => sum + (parseFloat(i.Qty) || 0), 0);
        const grandTotalOrig = data.reduce((sum, i) => sum + (parseFloat(i.OriginalTotal) || 0), 0);
        const grandTotalIDR = data.reduce((sum, i) => sum + (parseFloat(i.ConvertedTotal) || 0), 0);

        excelRows.push(["GRAND TOTAL", "", "", "", "", grandTotalQty, "", grandTotalOrig, grandTotalIDR]);

        const ws = XLSX.utils.aoa_to_sheet([
            ["Date", "Curr", "Invoice No", "DO No.", "Customer", "Qty", "Unit Price", "Total", "Total (IDR)"],
            ...excelRows
        ]);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "SalesItemWise");
        XLSX.writeFile(wb, "SalesItemWise.xlsx");
    };
    ...
    const headerTemplate = (data) => {
        return (
            <React.Fragment>
                <span className="image-text" style={{ fontWeight: 'bold' }}>Item: {data.ItemName}</span>
            </React.Fragment>
        );
    };
    ...
    const footerTemplate = (data) => {
        return (
            <React.Fragment>
                <td colSpan="5" className="text-end fw-bold" style={{ backgroundColor: '#fff9c4' }}>Subtotal</td>
                <td className="text-end fw-bold" style={{ backgroundColor: '#fff9c4' }}>
                    {calculateGroupTotal(data.ItemName, 'Qty')}
                </td>
                <td style={{ backgroundColor: '#fff9c4' }}></td>
                <td className="text-end fw-bold" style={{ backgroundColor: '#fff9c4' }}>
                    {formatNumber(calculateGroupTotal(data.ItemName, 'OriginalTotal'))}
                </td>
                <td className="text-end fw-bold" style={{ backgroundColor: '#fff9c4' }}>
                    {formatNumber(calculateGroupTotal(data.ItemName, 'ConvertedTotal'))}
                </td>
            </React.Fragment>
        );
    };
    ...
    // --- GRAND TOTAL FOOTER (Pinned at bottom) ---
    const footerGroup = (
        <ColumnGroup>
            <Row>
                <Column footer="GRAND TOTAL" colSpan={5} footerStyle={{ textAlign: 'right', fontWeight: 'bold', fontSize: '14px' }} />
                <Column footer={data.reduce((acc, item) => acc + (parseFloat(item.Qty) || 0), 0).toFixed(2)} footerStyle={{ textAlign: 'right', fontWeight: 'bold', fontSize: '14px' }} />
                <Column footer="" />
                <Column footer={formatNumber(data.reduce((acc, item) => acc + (parseFloat(item.OriginalTotal) || 0), 0))} footerStyle={{ textAlign: 'right', fontWeight: 'bold', fontSize: '14px' }} />
                <Column footer={formatNumber(data.reduce((acc, item) => acc + (parseFloat(item.ConvertedTotal) || 0), 0))} footerStyle={{ textAlign: 'right', fontWeight: 'bold', fontSize: '14px' }} />
            </Row>
        </ColumnGroup>
    );
            ...
            <Container fluid>
                ...
                <Card>
                    <DataTable
                        ...
                        groupRowsBy="ItemName" // Group by Item Name
                        sortMode="single"
                        sortField="ItemName"
                        sortOrder={1}
                        rowGroupHeaderTemplate={headerTemplate}
                        rowGroupFooterTemplate={footerTemplate}
                        footerColumnGroup={footerGroup} // Grand Total Footer
                        // --- GROUPING LOGIC END ---
                        className="p-datatable-gridlines"
                        emptyMessage="No records found"
                    >
                        {/* Hidden column strictly for grouping logic */}
                        <Column field="ItemName" header="Item Name" sortable hidden />
                        ...
                        {/* Display Customer Name in the grid since we are grouped by Item */}
                        <Column field="CustomerName" header="Customer Name" sortable={false} />
