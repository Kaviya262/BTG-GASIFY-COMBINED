...
const RequestAllocation = () => {
  ...
  const issueDateBodyTemplate = (rowData) => formatDateDisplay(rowData.issueDate);

  const loadRequestData = async (from, to) => {
    setLoading(true);
    try {
  ...
  const handleNew = () => {
    history.push("/warehouse/request/add");
  };
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const exportData = rows.map((row) => ({
        "Auto-Generated Number": row.autoGeneratedNumber,
      ...
      element.setAttribute("download", `Request_Export_${new Date().getTime()}.csv`);
  ...
  const handleSave = () => {
    try {
      ...
      console.log("Saving Request allocations:", updatedData);

      // Update the rows with the new allocation types
      setRows(updatedData);
      setEditingAllocation({});

      toast.success("Request allocations saved successfully");
    } catch (error) {
  ...
  const handlePost = () => {
    try {
      ...
      console.log("Posting Request allocations:", updatedData);

      // Update the rows with the new allocation types and status
      setRows(updatedData);
      setEditingAllocation({});

      toast.success("Request allocations posted successfully");
    } catch (error) {
      console.error("Error posting data:", error);
      toast.error("Failed to post Request data");
    }
  };
  ...
  const handleEdit = (rowData) => {
    history.push(`/warehouse/request/edit/${rowData.id}`, { requestData: rowData });
  };

  const handlePrint = (rowData) => {
    console.log("Printing request:", rowData.irNumber);
    toast.info(`Printing Request ${rowData.irNumber}...`);
  };
  ...
  document.title = "Warehouse Request Allocation | BTG Gas & Dashboard Template";

  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Warehouse" breadcrumbItem="Request" />
              ...
              <Card>
                ...
                {loading ? (
                  <div className="text-center p-5">
                    ...
                    <div className="mt-2 text-muted">Loading Request data...</div>
                  </div>
                ) : rows.length > 0 ? (
                  <>
                    <style>{`
                    ...
                    >
                      ...
                      <Column
                        ...
                        body={requestedDateBodyTemplate}
                        sortable
                        style={{ width: "12%", whiteSpace: "nowrap" }}
                        className="text-center"
                      />
                  ...
                  <div className="alert alert-info mt-3 text-center">
                    <p className="mb-0">No Request records found. Please use the search filter to view data.</p>
                  </div>
                )}
              </Card>
            </Col>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.95rem" }}>
                <tbody>
                  ...
                  <tr style={{ borderBottom: "1px solid #e9ecef" }}>
                    <td style={{ padding: "0.6rem 0.8rem", fontWeight: "bold", backgroundColor: "#f8f9fa" }}>Status</td>
                    <td style={{ padding: "0.6rem 0.8rem" }}>{selectedDetail.status}</td>
                  ...
                  <tr>
                    <td style={{ padding: "0.6rem 0.8rem", fontWeight: "bold", backgroundColor: "#f8f9fa" }}>Issue Date</td>
                    <td style={{ padding: "0.6rem 0.8rem" }}>{selectedDetail.issueDate || "Not Issued"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
